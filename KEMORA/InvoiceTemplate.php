...
class InvoiceTemplate extends FPDF {
    ...
    public function generateInvoice() {
        ...
        if ($this->data['payment_plan'] !== 'N/A') {
            $description .= "Payment Plan: " . $this->data['payment_plan'] . "\n";
        }
        
        // Add Mortgage Plan if available
        if ($this->data['mortgage_plan'] !== 'N/A') {
            $description .= "Mortgage Plan: " . $this->data['mortgage_plan'];
        }
        
        $this->MultiCell(90, 10, $description, 1, 'L');
        $currentY = $this->GetY();
        
        // Calculate height based on number of plans present
        $heightAdjustment = 20; // Base height for project name
        if ($this->data['payment_plan'] !== 'N/A') $heightAdjustment += 10;
        if ($this->data['mortgage_plan'] !== 'N/A') $heightAdjustment += 10;
        
        // Complete the row
        $this->SetY($currentY - $heightAdjustment);
