...
class EmailService {
    ...
    public function sendPaymentReceipt($email, $paymentDetails) {
        try {
            error_log("Starting to prepare invoice data");
            ...
            $invoiceData = [
                ...
                'mortgage_plan' => $paymentDetails['mortgagePlan'],
                'amount' => $paymentDetails['amount']
            ];
            error_log("Invoice data prepared: " . json_encode($invoiceData));

            // Generate PDF
            error_log("Generating PDF invoice");
            $pdf = new InvoiceTemplate($invoiceData);
            $pdf->generateInvoice();
            $pdfPath = sys_get_temp_dir() . '/Invoice_' . $paymentDetails['invoice_number'] . '.pdf';
            $pdf->Output('F', $pdfPath);
            error_log("PDF generated at: " . $pdfPath);

            // Set email parameters
            error_log("Setting up email parameters");
            ...
            // Attach PDF
            error_log("Attaching PDF to email");
            ...
            if (file_exists($pdfPath)) {
                unlink($pdfPath);
                error_log("Temporary PDF file cleaned up");
            }
            ...
            error_log("Email sending failed - Error details: " . $e->getMessage());
